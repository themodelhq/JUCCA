...
class PolicyEngine:
    ...
    def check_compliance(self, question: str, country: str = None, category: str = None):
        ...
        # 1. Check blacklisted keywords (store for later, don't block yet)
        ...
        if brand_issues:
            ...
            elif findings["decision"] == "Allowed":
                findings["decision"] = "Restricted"
        
        # 4. Add keyword issues last (as additional information)
        if keyword_issues:
    ...
    def _check_restricted_brands(self, question: str, country: str = None):
        ...
        for score, brand in matched_brands:
            ...
            if status == "forbidden":
                ...
                status_display = "ALLOWED WITH QC FOR FAKES"
            else:
                status_display = status.upper().replace("_", " ")
            
            issues.append(f"Brand '{brand.brand}' is {status_display}. {condition}")
