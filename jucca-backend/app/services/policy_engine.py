...
class PolicyEngine:
    ...
    def check_compliance(self, question: str, country: str = None, category: str = None):
        ...
        product_issues, product_statuses = self._check_prohibited_products(q, country, category)
        if product_issues:
            findings["issues"].extend(product_issues)
            findings["detected_products"] = list(set(findings.get("detected_products", []) + 
                                                     [p for p in product_issues if "Product '" in p]))
            
            # Only set to Prohibited if any product is actually prohibited/blocked
            has_prohibited = any(status in ['prohibited', 'blocked'] for status in product_statuses)
            has_restricted = any(status in ['restricted', 'license', 'registered', 'verified sellers only'] 
            ...
            # If only "open" or "allowed" statuses, keep decision as "Allowed"
        ...
        if brand_issues:
            ...
            # Check if any brand is FORBIDDEN (message says "is Blocked")
            has_forbidden = any("is Blocked" in issue for issue in brand_issues)
            
            if has_forbidden:
                findings["decision"] = "Blocked"
            # For non-forbidden brands, decision stays as "Allowed" (don't change to "Restricted")
        
        # 4. Process keyword issues - only if no brand was detected
        # Brand status takes priority over keyword matches
        if keyword_issues and not detected_brands:
            ...
            # Add keyword issues as notes, but don't override brand decision
            findings["issues"].extend(keyword_issues)
            findings["detected_keywords"] = detected_keywords
    ...
    def _check_blacklisted_keywords(self, question: str):
        ...
        for kw in keywords:
            ...
            # Skip disabled keywords (check description for "Status: disabled")
            if kw.description and 'status: disabled' in kw.description.lower():
                continue
    ...
    def _check_prohibited_products(self, question: str, country: str = None, category: str = None):
        ...
        Returns tuple of (issues, statuses) where statuses indicates whether products are allowed/prohibited.
        """
        ...
        for score, product in scored_matches:
            ...
            # Determine if this is actually an issue or just informational
            status = (product.status or "").lower()
            
            # Only create an issue if the product is actually restricted/prohibited
            # If status is "open" or "allowed", it's not an issue - it's allowed
            if status in ['open', 'allowed']:
                # Product is allowed - don't create an issue, just track the status
                statuses.append(status)
                # Optionally, we could create a positive message, but for now
                # we just don't create a negative issue
            else:
                # Product is restricted, prohibited, blocked, etc. - create an issue
