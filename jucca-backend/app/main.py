...
def initialize_default_policies():
    """Initialize default policy data from JSON file."""
    from sqlalchemy.orm import Session
    from .core.database import SessionLocal
    ...
    try:
        ...
        if existing_keywords > 0:
            print(f"Policy data already exists: {existing_keywords} keywords loaded")
            db.close()
            return
        ...
        if not policy_file.exists():
            print("Policy data file not found, using minimal defaults")
            db.close()
            return
        ...
        for item in policy_data.get("blacklisted_keywords", []):
        ...
        # Add restricted brands
        for item in policy_data.get("restricted_brands", []):
        ...
        for item in policy_data.get("prohibited_products", []):
        ...
        db.commit()
        print(f"Default policy data initialized: {len(policy_data.get('blacklisted_keywords', []))} keywords, "
              f"{len(policy_data.get('restricted_brands', []))} brands, "
              f"{len(policy_data.get('prohibited_products', []))} products")
...
async def create_user(
    ...
    log = SystemLog(
        ...
        metadata=json.dumps({"action": "create_user", "target_user": user_data.username})
    )
    db.add(log)
    db.commit()
...
async def update_user(
    ...
    log = SystemLog(
        ...
        user_id=current_user.id,
        metadata=json.dumps({"action": "update_user", "user_id": user_id, "updates": user_data.model_dump(exclude_none=True)})
    )
    db.add(log)
    db.commit()
...
async def delete_user(
    ...
    log = SystemLog(
        ...
        user_id=current_user.id,
        metadata=json.dumps({"action": "delete_user", "user_id": user_id, "deleted_username": username})
    )
    db.add(log)
    db.commit()
