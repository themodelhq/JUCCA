...
def seed_database():
    ...
    try:
        print("Loading comprehensive policy data from JSON...")
        ...
        existing_products = db.query(ProhibitedProduct).count()

        print(f"Existing database: {existing_keywords} keywords, {existing_brands} brands, {existing_products} products")

        # Always clear existing policy data to ensure fresh load
        if existing_keywords > 0 or existing_brands > 0 or existing_products > 0:
            print("Clearing existing policy data for fresh load...")
        ...
        print("=" * 60)
        print(f"\nComprehensive policy data loaded (version {json_version}):")
